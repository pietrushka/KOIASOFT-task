app.post('/api/extract', upload.single('file'), async (req, res) => {
    ...
    /* REVIEW: Use a validation library like zod to validate req.body and its properties. Please use early return with a 400 status code for invalid requests to reduce nesting if statements*/
    if (req.body) {
        const file = req.file;
        /* REVIEW: Destructuring can make this cleaner */
        ...
        if (requestID && project && idUser && user) {
            ...
            /* REVIEW: implement early returns */
            if (project === 'inkasso' && config.projects.hasOwnProperty(project) && file) {
                /* REVIEW: Hash logic seams not to be correct */
                const hashSum = crypto.createHash('sha256');
                const fileHash = idUser;
                /* REVIEW: Why is the fileName harcoded */
                ...
                /* REVIEW: Store the result of await db.hasUserRequestKey(idUser) in a variable, like userRequestExists, for clarity  */
                ...
                /* REVIEW: emails sending could be separate function 
                const sentStatus = {};
                /* REVIEW: Consider using for ... of loop and destructuring */
                for (let i = 0; i < debtCollectors.length ; i++) {
                    ...
                    /* REVIEW: requestKey creation could be separate funciton */
                    ...
                    if (!!(await db.setUserRequestKey(requestKey, idUser))
                        ...
                        /* REVIEW: entire email sending block should be a separate function */
                        /* prepare email */
                        const sendConfig = {
                            sender: config.projects[project].email.sender,
                            replyTo: config.projects[project].email.replyTo,
                            /* REVIEW: Replace the placeholder email subject with a more descriptive one */
                ...
                /* REVIEW: summary needs revist, should it be send? */
                const summaryConfig = {
